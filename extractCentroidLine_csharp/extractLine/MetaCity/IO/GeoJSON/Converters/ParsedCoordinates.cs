...
namespace MetaCity.IO.GeoJSON.Converters
{
    internal readonly struct ParsedCoordinates
    {
        ...
        public MultiPolygon ToMultiPolygon(GeometryFactory factory)
        {
            return _obj switch
            {
                null => factory.CreateMultiPolygon(),
                MultiPolygon multiPolygon => multiPolygon,
                _ => throw new InvalidOperationException("MultiPolygon is not supported (check SupportedTypes before calling any ToX methods)"),
            };
        }
