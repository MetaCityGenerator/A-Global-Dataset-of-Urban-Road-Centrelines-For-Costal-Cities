...
namespace MetaCity.DataProcessing
{
    public class InterchangeProcessing
    {
        ...
        public InterchangeProcessing(IList<LineString> roads, IList<Envelope> boxes)
        {
            ...
            foreach (Envelope box in boxes)
            {
                ...
                foreach (Coordinate c in connectPoints)
                    newLines.Add(new(new Coordinate[] { centroid, c }));
            }
            ...
            sw.Stop();
            Console.WriteLine("Finish processing all interchanges: {0}s", Math.Round((double)sw.ElapsedMilliseconds / 1000, 1));
        }

        public List<LineString> Results => results;

        private static IList<Envelope> GetCleanedEnvelopes(IList<Envelope> envelopes)
        {
            ...
            Geometry unionResult = UnaryUnionOp.Union(polygons);
            Geometry[] allgeoms;
            if (unionResult is Polygon)
                allgeoms = new Geometry[] {unionResult};
            else
                allgeoms = ((GeometryCollection)unionResult).Geometries;
        ...
        private static IList<LineString> GetCleanedRoads(IList<LineString> roads)
        {
            ...
            GeometryPrecisionReducer gpr = new(new(Math.Pow(10, PRECISION))) { 
                ChangePrecisionModel = true
            };
            HashSet<LineString> set = new();
        ...
        {
            Dictionary<(double x, double y), int> res = new();
        ...
        {
            Dictionary<LineString, int> res = new();
            for (int i = 0; i < roads.Count; i++)
                res.Add(roads[i], i);
            return res;
        }
        ...
        private ICollection<LineString> GetRoadsIntersectWithBox(Envelope box)
        {
            ...
            HashSet<LineString> roadsIntersectBoxHash = new(roadsIntersectBox);
            HashSet<int> roadsIntersectBoxVerticeHash = GetRoadsVertices(roadsIntersectBox, verticeDict);
            HashSet<LineString> neighRoads = new();
        ...
        {
            Polygonizer polygonizer = new() { IsCheckingRingsValid = false };
            ...
            MultiPolygon overpassGeometry = new(polygons);
        ...
        private static IEnumerable<Coordinate> GetVerticeCoords((double x, double y)[] verticeArr, IEnumerable<int> verticeIds)
        {
            ...
            foreach (int v in verticeIds)
                res.Add(new(verticeArr[v].x, verticeArr[v].y));
            return res;
        }
        ...
        private static Polygon EnvelopeToPolygon(Envelope envelope)
        {
            ...
            Polygon polygon = new(new(new Coordinate[]
            {
                new(minX, minY),
                new(maxX, minY),
                new(maxX, maxY),
