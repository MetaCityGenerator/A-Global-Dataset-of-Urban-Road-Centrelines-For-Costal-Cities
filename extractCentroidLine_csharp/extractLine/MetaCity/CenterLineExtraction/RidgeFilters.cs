...
namespace MetaCity.DataProcessing
{
    public class RidgeFilters
    {
        ...
        public static IList<LineSegment> GetRidgesInsideBuffer(Grid[,] grids, Envelope envelope, ICollection<Coordinate> coords, ICollection<LineSegment> ridges, double grid_size)
        {
            ...
            Dictionary<Coordinate, int> verticeDict = new();
            int i = 0;
            foreach (Coordinate c in coords)
            {
                vertices[i] = new(grids, envelope, grid_size, c.X, c.Y);
                verticeDict.Add(c, i);
                i++;
            }
...
double PRECISION)
        ...
        public static IList<LineSegment> RemoveDangles(IList<LineSegment> lineSegments)
        {
            ...
            Dictionary<Coordinate, int> frequency = new();
        ...
        {
            LineString this_line = new(new Coordinate[] { origin.Coordinate, destin.Coordinate });
            for (int i = Math.Min(origin.Row, destin.Row); i <= Math.Max(origin.Row, destin.Row); i++)
                ...
                {
                    Geometry g;
                    try { g = this_line.Intersection(grids[i, j].GridPolygon); }
                    catch (TopologyException)
                    {
                        try { g = OverlayNGRobust.Overlay(this_line, grids[i, j].GridPolygon,
                            NetTopologySuite.Operation.Overlay.SpatialFunction.Intersection); }
                        catch (TopologyException) { continue; }
                    }
